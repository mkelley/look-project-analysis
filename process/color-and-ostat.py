...
from quick_look import color_rho, setup_logger
...
for target in set(binned["target"]):
    ...
    trend = CometaryTrends(
        eph,
        binned[f"m{color_rho}"][i] * u.mag,
    ...
    ostat[i] = trend.ostat(k=4, dt=14, Phi=phase_HalleyMarcus, fixed_angular_size=True)

binned["ostat"] = ostat
binned.write("phot-binned.txt", format="ascii.fixed_width_two_line", overwrite=True)
...
colors.write("colors.txt", format="ascii.fixed_width_two_line", overwrite=True)
avg_colors.write("avg-colors.txt", format="ascii.fixed_width_two_line", overwrite=True)
