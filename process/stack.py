...
from lco_util import (assumed_gmr, locations, rho_labels,
...
def combine_photometry(phot, target, site, filter):
    ...
    for r in rho_labels:
        k = '{}'.format(r)
        _m, sw = np.average(phot['m' + k][i],
                            weights=phot['merr' + k][i]**-2,
                            returned=True)
        m['m' + k] = _m
        merr['merr' + k] = sw**-0.5
...
def stack_cluster(target, cluster, shape, site):
    ...
    for i, f in enumerate(cluster['file']):
        ...
        w.wcs.crval[0] += (eph['ra'][0] -
                           eph['ra'][i]).to_value('deg')
        w.wcs.crval[1] += (eph['dec'][0] - eph['dec']
                           [i]).to_value('deg')
        wcs.append(w)
...
def save_stack(wcs, avg, med, scale, target, site, cluster, filter, prefix):
    ...
    hdu[0].header['startmjd'] = (
        dates - cluster['exptime'] * u.s).min().mjd
    hdu[0].header['midmjd'] = date.mjd
    hdu[0].header['stopmjd'] = (
        dates + cluster['exptime'] * u.s).max().mjd
    ...
    hdu[0].header['gain'] = scale.mean(
    )**-1, 'mean eff. gain per image'
    hdu[0].header['ra'] = cluster['ra'].mean(), 'mean observed RA, deg'
    hdu[0].header['dec'] = cluster['dec'].mean(
    ), 'mean observed Dec, deg'
    ...
    hdu[0].header['nimages'] = len(
        cluster), 'number of images combined'
...
def update_binned_photometry(cluster, target, site, filter, prefix, binned, phot):
    ...
    tab.write('phot-binned.txt', format='ascii.fixed_width_two_line',
              overwrite=True)
    ...
    for (target, site, filter), rows in groupby(phot, grouper):
        ...
        for cluster in [group[clusters == c] for c in set(clusters)]:
            ...
            process_stack = (
                ...
                or not any(f'{prefix}_{label}.fits' in existing_files
                           for label in ('avg', 'med'))
                # any files from this cluster not associated with this prefix?
                or any(not associations.get(f, [''])[0].startswith(prefix)
                       for f in cluster['file'])
            )
            if process_stack:
                ...
                if args.process:
                    wcs, avg, med, scale = stack_cluster(
                        target, cluster, shape, site)

                    stack_files = save_stack(wcs, avg, med, scale, target, site,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no-processing', dest='process', action='store_false',
