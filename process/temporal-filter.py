...
parser.add_argument(
    "-v", dest="level", action="store_const", const="DEBUG", help="console verbose mode"
)
parser.add_argument(
    "-q", dest="level", action="store_const", const="WARNING", help="console quiet mode"
)
args = parser.parse_args()

logger = quick_look.setup_logger(__file__, args.level)
logger.info("Start")
...
half_box = 230
y0, x0 = 537, 537
cutout = np.s_[y0 - half_box : y0 + half_box + 1, x0 - half_box : x0 + half_box + 1]
...
for target, group in groupby(sorted(stacks.items()), key=grouper):
    ...
    group = list(group)
    logger.debug("%s %d stack%s", target, len(group), "" if len(group) == 1 else "s")
    ...
    for i, (stack_file, sources) in enumerate(group):
        filtered_file = stack_file.replace("stacks/", "processed/").replace(
            ".fits", "_tf.fits"
        )
        plot_file = filtered_file.replace(".fits", ".jpg").replace(
            "processed/", "plots/"
        )
